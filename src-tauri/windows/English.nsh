; Custom NSIS English strings for Hytale Server Manager.
; Based on Tauri's default English.nsh — overrides the "app is running" kill dialog
; so users are warned to stop servers before the installer force-closes the app.
...
LangString appRunning ${LANG_ENGLISH} "{{product_name}} is running! Stop any running servers, close the manager, then try again."
LangString appRunningOkKill ${LANG_ENGLISH} "{{product_name}} is running!$\n$\nStop any running servers in the manager first.$\nClick OK to force-close it and continue."
...
LangString failedToKillApp ${LANG_ENGLISH} "Failed to close {{product_name}}. Stop any running servers, close the manager, then try again."
