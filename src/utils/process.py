...
output back to the UI through callbacks.
"""
...
def run_in_thread(
    ...
    *,
    ...
    """
    Launch *cmd* in a background thread.

    * ``on_output(line)`` is called for every line of combined stdout/stderr.
    * ``on_done(returncode)`` is called when the process exits.
...
def run_capture(
    ...
    *,
    ...
    """
    Run *cmd* synchronously, capture combined output, return ``(returncode, output)``.
    """
    try:
